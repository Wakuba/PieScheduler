...
export default function FanShape({
  ...
  //円弧の始まりの座標;
  const outerStartX =
    cx + outerRadius * Math.sin(((startDegree + 90) / 180) * Math.PI);
  const outerStartY =
    cy - outerRadius * Math.cos(((startDegree + 90) / 180) * Math.PI);

  //円弧の終わり座標;
  const outerFinishX =
    cx + outerRadius * Math.sin(((finishDegree + 90) / 180) * Math.PI);
  const outerFinishY =
    cy - outerRadius * Math.cos(((finishDegree + 90) / 180) * Math.PI);
  ...
  //内円
  const innerStartX =
    cx + innerRadius * Math.sin(((startDegree + 90) / 180) * Math.PI);
  const innerStartY =
    cy - innerRadius * Math.cos(((startDegree + 90) / 180) * Math.PI);

  //円弧の終わり座標;
  const innerFinishX =
    cx + innerRadius * Math.sin(((finishDegree + 90) / 180) * Math.PI);
  const innerFinishY =
    cy - innerRadius * Math.cos(((finishDegree + 90) / 180) * Math.PI);

  //弧の中点
  const outerMediumX =
    cx +
    outerRadius *
      Math.sin((((startDegree + finishDegree) / 2 + 90) / 180) * Math.PI);
  const innerMediumX =
    cx +
    innerRadius *
      Math.sin((((startDegree + finishDegree) / 2 + 90) / 180) * Math.PI);
  const outerMediumY =
    cy -
    outerRadius *
      Math.cos((((startDegree + finishDegree) / 2 + 90) / 180) * Math.PI);
  const innerMediumY =
    cy -
    innerRadius *
      Math.cos((((startDegree + finishDegree) / 2 + 90) / 180) * Math.PI);
